        ...
        public bool connect(string name, string password)
        {
            ...
            using (SqlDataAdapter adapter = new SqlDataAdapter("SELECT * FROM [User]", connection))
            {
                ...
                foreach (DataRow row in rows)
                {
                    ...
                    if (name.Equals(id) && password.Equals(pwd))
                    {
            ...
            return Program.login_succeed;
    }

        private void btn_connect_Click(object sender, EventArgs e)
        {
            this.stateconnexion = false;
           this.stateconnexion = this.connect(login_ID.Text, login_PWD.Text);
        }
